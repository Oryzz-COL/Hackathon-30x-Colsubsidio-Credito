...
Cédula sintética o lote
  ...
  → libro de evidencia
  → orden interno por familias independientes
  → producto + condición + razón + canal + momento
  → revisión humana + entrega demostrativa
```

`lib/enrichment/engine.ts` es el orquestador. Los conectores implementan un contrato común y devuelven `ExternalSignal`; no modifican directamente la clasificación. `lib/enrichment/policy.ts` excluye señales sin consentimiento, vencidas o sensibles. `lib/enrichment/scoring.ts` permite una sola contribución por familia para evitar contar tres veces el mismo dato. El orden numérico es interno; la interfaz comunica niveles cualitativos y evidencia, no una certeza absoluta.
